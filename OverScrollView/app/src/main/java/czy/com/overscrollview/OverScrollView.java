package czy.com.overscrollview;
...
//        setOverScrollMode(OVER_SCROLL_ALWAYS);
    ...
    protected void onFinishInflate() {
        ...
        Log.e(TAG, "onFinishInflate: mCard1 = " + mCard1);
    }
    ...
    public void computeScroll() {
        ...
        if (mScroller.computeScrollOffset()) {
            ...
            Log.e(TAG, "computeScroll: oldX = " + oldX + " , oldY = " + oldY + " , x = " + x + " , y = " + y + " oldX - x = " + (oldX - x));
            if (oldX != x || oldY != y) {
...
//                    if (canOverscroll) {
                        if (true) {
                            final int pulledToX = oldX + deltaX - mTransX;
                            Log.e(TAG, "onTouchEvent: Over Scroll oldX = " + oldX + " deltaX = " + deltaX + " , pulledToX = " + pulledToX + " , range = " + range);
                            if (pulledToX > range) {
                                // transX 移动
                                transXChild(pulledToX - range);
                            }
                        }
                    }
...
//                            }
    ...
    private void transXChild(int detalX) {
        ...
        Log.e(TAG, "transXChild: mTransX = " + mTransX);
        mRecycleView.setTranslationX(mTransX);
        mCard1.setTranslationX(mTransX * 4 / 5);
        mCard2.setTranslationX(mTransX * 3 / 5);
        mCard3.setTranslationX(mTransX * 2 / 5);
    }

    private void resetTransXChild() {
        mTransX = 0;
        mRecycleView.setTranslationX(0);
        mCard1.setTranslationX(0);
        mCard2.setTranslationX(0);
        mCard3.setTranslationX(0);
    }

    AnimatorSet mAnimatorSet;

    private void animBanTransXChild() {
        if (mAnimatorSet != null && mAnimatorSet.isRunning()) {
            mAnimatorSet.cancel();
        }
        ...
        ObjectAnimator recycleViewTransXAnim = ObjectAnimator.ofFloat(mRecycleView, "translationX", mRecycleView.getTranslationX(), 0);
        ObjectAnimator card1TransXAnim = ObjectAnimator.ofFloat(mCard1, "translationX", mCard1.getTranslationX(), 0);
        ObjectAnimator card2TransXAnim = ObjectAnimator.ofFloat(mCard2, "translationX", mCard2.getTranslationX(), 0);
        ObjectAnimator card3TransXAnim = ObjectAnimator.ofFloat(mCard3, "translationX", mCard3.getTranslationX(), 0);

        recycleViewTransXAnim.addUpdateListener(new ValueAnimator.AnimatorUpdateListener() {
            @Override
            public void onAnimationUpdate(ValueAnimator animation) {
                float val = (float) animation.getAnimatedValue();
                mTransX = (int) val;
//                mRecycleView.setTranslationX(mTransX);
                Log.e(TAG, "onAnimationUpdate: val = " + val);
            }
        });

        mAnimatorSet.playTogether(recycleViewTransXAnim, card1TransXAnim, card2TransXAnim, card3TransXAnim);
        mAnimatorSet.setDuration(300);
        mAnimatorSet.start();

        Log.e(TAG, "animBanTransXChild: ");
    }
